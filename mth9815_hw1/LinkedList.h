...
public:
	...
	LinkedList() : size(0), head(new Node<T>(nullptr)) {}

	// Add the specified element at the end of the list
	void Add(T& value) {
		Node<T>* running_node = head;
		while (running_node->next_node != nullptr) {
			running_node = running_node->next_node;
		}
		running_node->next_node = new Node<T>(value, nullptr);
		size++;
	}
	...
	void Insert(T& value, int index) {
		...
		Node<T>* running_node = head;
		while (index != 0 && running_node->next_node != nullptr) {
			running_node = running_node->next_node;
			index--;
		}
