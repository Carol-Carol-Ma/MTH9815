...
public:
    //Future ctor
    Future(string _productId, FutureType _futureType, string _ticker, float _price, /*int _size, */date _maturityDate);
    Future();

    // return the type of future
    FutureType GetFutureType() const;

    // return the underlying asset ticker
    string GetTicker() const;

    // return the strike price of the future
...
private:
    ...
    string ticker; // underlying asset ticker variable
    float price;// predetermined future price
    //int size;
    date maturityDate;
};

Future::Future(string _productId, FutureType _futureType, string _ticker, float _price, date _maturityDate): Product(_productId, FUTURE) {
...
{
    output << future.ticker << " " << future.price << " " << future.GetMaturityDate();
    return output;
}
