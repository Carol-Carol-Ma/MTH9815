...
template <typename k, typename v> struct HashNode {
	k key; v value;
	HashNode(k _k = k(), v _v = v()) : key(_k), value(_v) {};
...
public:
    ...
    typedef typename list<HashNode<K, V> >::iterator iterator;
    typename list<HashNode<K, V> >::iterator find(K key, list<HashNode<K, V> >* ls)
	{
		auto iter = ls->begin();
		while (iter != ls->end()) {
			// cout << iter->key;
